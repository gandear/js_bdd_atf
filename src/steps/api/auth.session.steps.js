...
When('I authenticate with valid credentials', async ({ apiClient, apiState }) => {
  const { email, password } = TestDataFactory.getValidCredentials()[0];
  const { res, json } = await apiClient.loginAndSetToken({ email, password });
  apiState.res = res; apiState.json = json;
});

Then('the client holds a bearer token', async ({ apiClient, apiState }) => {
